...
class TFProcess:
    def __init__(self):
        ...
        gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=2/3.0)
    ...
    def init_net(self, planes, probs, winner):
        ...
        print("tensorflow has initial all variable in this net .")
        self.session.run(tf.global_variables_initializer())
        print("all variables have been initailized .")
    ...
    def measure_loss(self, batch, training=False):
        ...
        #print(len(batch[2]))
        ##################
        policyOutput= self.session.run(self.y_conv, feed_dict={self.training: training,
                           self.planes: batch[0],
                           self.probs: batch[1],
                           self.winner: batch[2]})
        print("policy output is ",policyOutput)
        valueOutput =  self.session.run(self.y_conv, feed_dict={self.training: training,
                           self.planes: batch[0],
                           self.probs: batch[1],
                           self.winner: batch[2]})
        print("value output is ",valueOutput)
