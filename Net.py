...
class Net :
    ...
    def __init__(self):
        ...
        self.inputPlaceHolder = tf.placeholder(dtype=tf.float32,shape=[None,17,19,19])
    ...
    def lossAndOptConstruction(self):
        ...
        #self.opt = opt = tf.train.AdamOptimizer(learning_rate=0.0001).minimize(self.lossT)
        update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
        with tf.control_dependencies(update_ops):
            self.opt = opt = tf.train.MomentumOptimizer(momentum=0.9,learning_rate=self.learningRate,
